...
int last_obs_position = WIDTH + 300;
...
void setObs(int c) {
    int gap = 300;
    obstacle_position_x[c] = last_obs_position + gap;
    last_obs_position = obstacle_position_x[c];
    obstacle_position_y[c] = -200 + randNum(-200, 200);
    iSetSpritePosition(&obs[c], obstacle_position_x[c], obstacle_position_y[c]);
    if(c == 4) {
        last_obs_position = WIDTH + 300;
    }
}
...
void gameOver() {
    ...
    iShowLoadedImage(300, 350, &gameover);
}
