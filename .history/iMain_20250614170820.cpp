...
void iDraw()
{
    ...
    iText(200, 100, "text", GLUT_BITMAP_8_BY_13);
...
void setObstacle(int i) {
    for(int j = 0; j<=i; j++) {
        iInitSprite(&obs[j], -1);
        iChangeSpriteFrames(&obs[j], frames_obstacle, j+1);
        iSetSpritePosition(&obs[j], 1200, -200 + j*100);
        iScaleSprite(&obs[j], 0.7);
    }
...
void moveObstacle(int i) {
    ...
    for(int j = 0; j<=i; j++) {
        iSetSpritePosition(&obs[j], obstacle_position_x, -200);
    }
}
